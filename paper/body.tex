...
\caption{Far bin ($n=120$; Llama-4 $n=118$), off-center balls, 95\% bootstrap CIs. No current frontier
...
Trained on one sport and tested zero-shot on the other (12 soccer matches, 250k frames;
6 basketball games, 231k frames), the Deep Sets transfers asymmetrically---basketball$\rightarrow$soccer
...
(positions only). A basketball court is $\sim$1/16 the area of a pitch ($\sim$4$\times$
shorter linearly), so in normalized coordinates velocities differ 3--4$\times$; the
learned use of velocity is scale-bound and does not cross, while scale-free positions do. The interpretable geometry model, which rescales a
