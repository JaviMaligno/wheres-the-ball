...
two of the models (Azure-hosted GPT-5.4, Llama-4-Maverick) are versioned snapshots whose
behaviour drifts, so ``no current VLM beats the baseline'' is a dated claim, not a general
one. In the far bin, ``off-center ball'' is partly entangled with unusual (transition/wide)
camera framing. Learned models are single-architecture; the reported $\pm$ is inter-match,
with inter-seed variance separately small ($\le0.002$). The Claude Opus 4.8 / Sonnet 4.6
runs were incomplete (API-credit limited) and are excluded from the headline comparison.
